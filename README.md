...
| [`baton`](./baton/) | Long-running multi-phase build orchestrator. `baton-runner` queues specs as implement→review→fix units; `baton-runner-multi-agent` uses multi-agent dev teams per phase; `baton-pass` writes append-only session notes. |
