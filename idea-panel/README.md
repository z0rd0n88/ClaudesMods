...
Generative sibling of [`multi-agent-review`](https://github.com/z0rd0n88/ClaudesMods/tree/main/multi-agent-review): instead of a panel of reviewers finding faults in an existing artifact, this panel of lenses attacks a topic from distinct cognitive frames (first-principles, jobs-to-be-done, inversion, effectuation, …) so the same problem space gets explored from angles a single pass would collapse into one.
...
claude plugin install idea-panel
```
...
| Sharpen the chosen idea | `idea-autopsy:iterate-to-v2` | [sibling plugin in ClaudesMods](https://github.com/z0rd0n88/ClaudesMods/tree/main/idea-autopsy) |
| Formalize into a spec | `product-management:write-spec` / `to-prd` | separate skills |
| Pressure-test before committing | `idea-autopsy:evaluate-proposal-harsh` | [sibling plugin in ClaudesMods](https://github.com/z0rd0n88/ClaudesMods/tree/main/idea-autopsy) |
