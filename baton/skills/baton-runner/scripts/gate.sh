...
# MYPY_TARGET: the path mypy type-checks. Override per-project (e.g. `src/foo`,
# `src`, `.`); defaults to `src`. Set in the worktree env or via the manager at
# pre-flight; documented in REFERENCE.md.
MYPY_TARGET="${MYPY_TARGET:-src}"
...
run mypy        uv run mypy "$MYPY_TARGET"   || fail=1
